document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function clearError(field) {
      const wrapper = field.closest('.form-group, .field-wrapper, [data-field]') || field.parentElement;
      const errorEl = wrapper.querySelector('.' + ERROR_CLASS);
    ...
    function validateField(field) {
      ...
      } else if (type === 'tel' && value.trim() && !validators.tel(value)) {
        errorMessage = errorMessages.tel;
      } else if (field.hasAttribute('minlength') && value.trim() && !validators.minlength(value, field.getAttribute('minlength'))) {
        errorMessage = 'Please enter at least ' + field.getAttribute('minlength') + ' characters.';
      } else if (field.hasAttribute('maxlength') && value.trim() && !validators.maxlength(value, field.getAttribute('maxlength'))) {
        errorMessage = 'Please enter no more than ' + field.getAttribute('maxlength') + ' characters.';
      } else if (field.hasAttribute('data-validate')) {
        const customType = field.getAttribute('data-validate');
        if (validators[customType] && !validators[customType](value)) {
          errorMessage = errorMessages[customType] || errorMessages.default;
        }
      }
  ...
  function initScrollAnimations() {
    const animatedElements = Array.from(document.querySelectorAll(ANIMATION_SELECTOR));
    ...
    if (!document.querySelector('#scroll-animation-styles')) {
      ...
      style.textContent = [
        '.fade-in, [data-animate], .animate-on-scroll {',
        '  opacity: 0;',
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'f7485f98-1bba-4761-ae5d-f359f1c6e87a',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'f7485f98-1bba-4761-ae5d-f359f1c6e87a';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f7485f98-1bba-4761-ae5d-f359f1c6e87a';
